...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.conditional_layers = getattr(config, "conditional_layers", None)
        if self.conditional_size and not self.conditional_layers:
...
def get_sinusoidal_positions_embeddings(input,output_dim):
    ...
    embeddings = torch.reshape(embeddings, (-1, seq_len, output_dim)).to(input.device)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        elif self.position_embedding_type == "transformer_xl":
            pass
            # self.relative_positions_encoding = relative_position_encoding(max_length=config.max_position_embeddings,
            #                                                               depth=self.attention_head_size,
            #                                                               max_relative_position=config.max_relative_position)
    ...
    def forward(
        ...
        if self.position_embedding_type == 'roformer':
            sinusoidal_pos = get_sinusoidal_positions_embeddings(hidden_states,self.attention_head_size)
        ...
        #if self.position_embedding_type == "transformer-xl":
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.conditional_size = getattr(config, "conditional_size", None)
        self.conditional_layers = getattr(config, "conditional_layers", None)
        if self.conditional_layers and self.conditional_size:
            config.conditional_size = None
            layers_stack1 = [BertLayer(config) for _ in range(config.num_hidden_layers-self.conditional_layers)]
            config.conditional_size = self.conditional_size
            layers_stack2 = [BertLayer(config) for _ in range(self.conditional_layers)]
            self.layer = nn.ModuleList(layers_stack1+layers_stack2)
        else:
