...
load_path = 'Qianyan_fgm928.pth'
output_file = 'Qinyan_train_fgm.json'
...
class CONLINER(nn.Module):
    def __init__(self, model_path):
        ...
        #self.standerembed = nn.Embedding(1, 128)
        #self.addliner = nn.Embedding(1, 128)
        self.bert = BertModel.from_pretrained(model_path, config=Config)
        
    def forward(self, source_input_ids, source_attention_mask, source_token_type_ids,target_input_ids, target_attention_mask, target_token_type_ids, c):
        source_x1 = self.bert(source_input_ids, attention_mask=source_attention_mask, token_type_ids=source_token_type_ids)
        target_x1 = self.bert(target_input_ids, attention_mask=target_attention_mask, token_type_ids=target_token_type_ids)
...
def train(dataloader, model, loss_fn, optimizer, scheduler):
    ...
    correct = 0
    #test way
    for line in  tqdm(enumerate(dataloader)):
        batch=line[0]
        ...
        pred = model(source_input_ids, source_attention_mask, source_token_type_ids,target_input_ids, target_attention_mask, target_token_type_ids, c)
        ...
        if use_FGM:
            fgm.attack()
            loss_adv = loss_fn(model(source_input_ids, source_attention_mask, source_token_type_ids,target_input_ids, target_attention_mask, target_token_type_ids, c), y)
            loss_adv.backward()
            fgm.restore()
        ...
        model.zero_grad()
        if batch % 200 == 0:
            loss, current = loss.item(), batch * len(source_input_ids)
...
def test(dataloader, model):
    ...
    with torch.no_grad():
        for line in  tqdm(enumerate(dataloader)):
            ...
            TP_a += ((pred_result == 1) & (y == 1) ).type(torch.float).sum().item()
            TN_a += ((pred_result == 0) & (y == 0) ).type(torch.float).sum().item()
            FN_a += ((pred_result == 0) & (y == 1) ).type(torch.float).sum().item()
            FP_a += ((pred_result == 1) & (y == 0) ).type(torch.float).sum().item()
    p_a = TP_a / (TP_a + FP_a)
    r_a = TP_a / (TP_a + FN_a)
