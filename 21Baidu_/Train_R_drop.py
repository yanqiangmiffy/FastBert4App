...
load_path = 'Qianyan_use_alltrain.pth'
...
def train(dataloader, model, loss_fn, optimizer, scheduler):
    ...
    for batch, (data, y, c, _) in tqdm(enumerate(dataloader)):
        ...
        pred1 = model(input_ids, attention_mask, token_type_ids, c)
        pred2 = model(input_ids, attention_mask, token_type_ids, c)

        correct += (pred1.argmax(1) == y).type(torch.float).sum().item()

        loss = RDrop(pred1,pred2, y)
        loss.backward()
