...
load_path = 'Qianyan_use_alltrain0927.pth'
output_file = 'Qinyan_train_prob.json'
...
if __name__ == '__main__':
    ...
    optimizer = AdamW(teacher_model.parameters(), lr=learning_rate)
    ...
    for t in range(epochs):
        ...
        if test_F1 > F1max:
            F1max = test_F1
            torch.save(teacher_model.state_dict(), load_path)
    ...
    # print("predicting all data")
    # id_prob = {}
    # with torch.no_grad():
    #     for data, _, c, idx in all_dataloader:
    #         input_ids = data['input_ids'].to(device)
    #         attention_mask = data['attention_mask'].to(device)
    #         token_type_ids = data['token_type_ids'].to(device)
    #         c = c.to(device)
    #         pred = teacher_model(input_ids, attention_mask, token_type_ids, c)
    #         p_T = F.softmax(pred/Temperature, dim=-1)
    #         for d_, prob in zip(idx, p_T):
    #             id_prob[str(d_.item())] = prob.cpu().numpy().tolist()
    #
    # with open(output_file, 'w') as f:
    #     f.write(json.dumps(id_prob))